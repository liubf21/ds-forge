...
> **别和 DeepSeek 搞混**：`reasoning_content` 是 **DeepSeek-reasoner** 的专有字段（OpenAI Chat Completions 不返回它）。DeepSeek 在响应的 `message.reasoning_content` 里给出推理全文，且官方明确要求**下一轮不要把它带回去**——和 Anthropic 的"必须原样回传"恰好相反。这个项目走 DeepSeek，吃到的是这套行为。
>
> ```jsonc
> // DeepSeek-reasoner 响应
> { "choices": [{ "message": {
>   "reasoning_content": "用户想知道北京天气...",  // DeepSeek 专有，不回传
