...
```
findAgentsMd({cwd, global})  → 从 cwd 向上走到 git root，收集链上所有 AGENTS.md
...
| `AgentSession.agentsMd` | `boolean \| AgentsMdOptions` | **开** | coding agent 预设，符合业界自动读取行为 |
| TUI `--global-agents` | flag | **关** | 显式开启 global AGENTS.md |

想用 global AGENTS.md：`new Forge({ agentsMd: { global: true } })` /
`AgentSession.open({ agentsMd: { global: true } })` / `npm run tui -- --global-agents`。
...
教训 & 修正：读用户全局配置当默认行为是越权。改为**项目级自动发现、global 显式
opt-in（`global: true`）**。更可预测，也顺带修了测试 —— 不靠改测试，靠改对默认。
...
`examples/agents_md_test.ts`，14 用例：空/缺失、git 边界向上走、无 git 不乱爬、空文件跳过、
section 格式与优先级、`AGENTS.override.md` 每目录优先、global `~/.agents` + override、32KiB 截断、
Forge 注入（默认关）、AgentSession 默认开 + clear 轮回、`agentsMd:false` 关闭。
