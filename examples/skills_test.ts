#!/usr/bin/env npx tsx
/**
 * Skill system test suite. Zero external deps, no API calls — uses Node assert
 * and a temp directory of fixture SKILL.md files.
 *
...
async function main() {
  ...
  await check("discoverSkills: project layer, no user dir", () => {
    const root = fixtureDir();
    const reg = discoverSkills({ cwd: root, includeUser: false });
  ...
  await check("discoverSkills: scans cwd up to git root, nearest wins", () => {
    ...
    const reg = discoverSkills({ cwd: sub, includeUser: false });
  ...
  await check("discoverSkills: extra dirs win on name collision", () => {
    ...
    const reg = discoverSkills({ dirs: [a, b], cwd: "/no/project", includeUser: false });
    assert.strictEqual(reg.get("dup")!.body, "from-a");
  });

  await check("renderSkill: ${arguments} and ${SKILL_DIR}", () => {
    const reg = discoverSkills({ cwd: fixtureDir(), includeUser: false });
  ...
  await check("Forge: skills register `skill` tool + inject catalog", () => {
    ...
    const registry = discoverSkills({ cwd: fixtureDir(), includeUser: false });
